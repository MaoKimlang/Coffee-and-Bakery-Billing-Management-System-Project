...
BEGIN
    SELECT 
        ...
        oi.quantity,
        i.price,
        (i.price * (1-oi.discount))* oi.quantity AS totalPrice
    FROM 
        Orders o
...
BEGIN
    UPDATE Orders
    SET totalAmount = (
        SELECT i.price * (1-oi.discount)* oi.quantity
        FROM Order_Items oi
        JOIN items i ON oi.itemID = i.itemID
...
DELIMITER ;
INSERT INTO Cashiers (cashierID, cashierName, shift)
VALUES 
('cashier1', 'The Weekend', 'morning'),
('cashier2', 'Lana Del rey', 'afternoon'),
('cashier3', 'Sun Jae', 'evening');